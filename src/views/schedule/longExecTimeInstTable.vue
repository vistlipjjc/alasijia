            ...
            <Row style="margin-top: 10px">
                <Col span="18">
                    <j-table  :showColumn="false" autoSearch :showRefresh="false"  :input-col="5"
                              :server="server.queryLongExecTimeInst"  pagination :queryForm="queryForm" :defaultParams="tableParam"
                              :columns-list="longExecTimeInstColumns" v-model="longExecTimeInstData"
    ...
    export default {
        ...
        data() {
            return {
                ...
                server: {
                    queryLongExecTimeInst: this.$services.schService.jobInstMonitorRestController.queryLongExecTimeInst,
                },

                longExecTimeInstColumns: [
                    {title: '任务类型', align: 'center', key: 'name_'},
                    {title: '任务名称', align: 'center', key: 'job_name_', ellipsis: true},
                    {title: '开始时间', align: 'center', key: 'start_time',
                        render: (h, params) => {
                            return h('span', [
                                h('span', this.formatDate(params.row.start_time))
                            ]);
                        }},
                    {title: '执行时间(ms)', align: 'center', key: 'exec_time_'},
                    {title: '平均执行时间(ms)', align: 'center', key: 'AVG_EXEC_TIME_'},
                ],

                longExecTimeInstData: [],
            }
        },
        ...
        methods: {
            ...
            init() {
                this.$net.send({
                    ...
                    this.queryForm.__LIKES_jobName = this.jobCodeList[0].name
                    this.queryForm.__LIKES_jobCode = this.jobCodeList[0].value
                    this.tableParam = this.queryForm;
