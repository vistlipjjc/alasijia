...
<template>
    <Modal 	width="60%"
              v-model="selectCustomConditionModal"
              title="选择条件"
              :transfer="false"
              :mask-closable = "false"
              @on-ok="selectCustomConditionSave"
              @on-cancel="cancel">
        <template>
            <template>
...
<script>
    export default {
        name: "select-custom-condition",
        props:{
        },
        data() {
            return {
                selectCustomConditionModal: false,
                transfer: true,
                condition: '',
                tables: [],
                search: '',
        ...
        methods: {
            ...
            openSelectCustomConditionModal(data){
                this.init(data);
                this.selectCustomConditionModal = true;
            },
            cancel(){
                this.$emit('closeSelectCustomCondition', true);
            },

            selectCustomConditionSave(){
                let data = {condition : this.condition};
                this.$emit('closeSelectCustomCondition', false, data);
            },
