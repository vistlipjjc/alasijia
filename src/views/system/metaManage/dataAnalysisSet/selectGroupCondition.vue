...
<template>
    <Modal 	width="50%"
              v-model="selectGroupConditionModal"
              title="选择条件"
              :transfer="true"
              :mask-closable = "false"
              @on-ok="selectGroupConditionSave"
              @on-cancel="cancel">
        <template>
            <template>
            ...
            <template>
                <Tabs value="name1" ref="condituonColumn">
                    <TabPane label="原始列" name="name1">
                        ...
                        <Row>
                            <Table @on-row-dblclick="onHavingDblclick" :columns="selectGroupConditionColumns" :data="searchColumnsData" height="300"></Table>
                        </Row>
                    </TabPane>
...
<script>
    export default {
        name: "select-group-condition",
        props:{
        },
        data() {
            return {
                selectGroupConditionModal: false,
                transfer: true,
                condition: '',
                tables: [],
                search: '',
                selectGroupConditionColumnsData: [],
                searchColumnsData: [],
                selectGroupConditionColumns: [
                    {
                        title: '列名',
                        key: 'code'
                    },
        ...
        methods: {
            init(data){
                ...
                this.selectGroupConditionColumnsData = data.groupColumns;
                this.searchColumnsData = JSON.parse(JSON.stringify(this.selectGroupConditionColumnsData));
            },
            openSelectGroupConditionModal(data){
                this.init(data);
                this.selectGroupConditionModal = true;
            },
            cancel(){
                this.$emit('closeSelectGroupCondition', true);
            },

            selectGroupConditionSave(){
                let data = {condition : this.condition};
                this.$emit('closeSelectGroupCondition', false, data);
            },
            ...
            searchMethod(){
                if(this.search && this.search != ''){
                    this.searchColumnsData = this.filterMethod (this.selectGroupConditionColumnsData, 'code', 'name', this.search);
                }
                else{
                    this.searchColumnsData = JSON.parse(JSON.stringify(this.selectGroupConditionColumnsData));
                }
