                ...
                >
                    ...
                    <FormItem label="用户手机：" prop="cellphone" >
                        ...
                        <div style="display:inline-block;width:204px;">
                            <Input :maxlength="11" v-model="userForm.cellphone" @on-keydown="hasChangePhone"></Input>
                        </div>
        ...
        <Modal v-model="editPasswordModal" :closable='false' :mask-closable=false :width="500">
            ...
            <Form ref="editPasswordForm" :model="editPasswordForm" :label-width="100" label-position="right" :rules="passwordValidate">
                ...
                <FormItem label="新密码" prop="newPass">
                    <Input type="password" v-model="editPasswordForm.newPass" :maxlength="16" placeholder="请输入新密码，至少6位字符" ></Input>
                </FormItem>
                <FormItem label="确认新密码" prop="rePass">
                    <Input type="password" v-model="editPasswordForm.rePass" :maxlength="16" placeholder="请再次输入新密码" ></Input>
                </FormItem>
            </Form>
            <div slot="footer">
...
export default {
        ...
        return {
            ...
            passwordValidate: {
                oldPass: [
                    ...
                    //         if(!this.opt){
                    //             this.$net.send({
                    //                 server: this.$services.sysService.userRestController.checkOldPass,
                    //                 data: {userId: JSON.parse(localStorage.getItem('user')).staffCode, password: this.editPasswordForm.oldPass}
                    //             }).then((data) => {
                    //                 if (!data.valid) {
                ...
                newPass: [
                  { required: true, message: '请输入英文大写、小写和数字的组合密码', min: 6, trigger: 'blur' },
                  { validator:
                    (rule, value, callback) => {
    ...
    methods: {
        ...
        saveEditPass () {
            this.$refs['editPasswordForm'].validate((valid) => {
                if (valid) {
                    ...
                    try {
                      this.$net.send({
                        server: this.$services.sysService.userRestController.savePass,
                        data: {
                          userId: JSON.parse(localStorage.getItem('user')).staffCode,
                          // oldPassword: this.editPasswordForm.oldPass,
                          // newPassword: this.editPasswordForm.newPass,
                          oldPassword: md5(this.editPasswordForm.oldPass),
                      ...
                      this.savePassLoading = false
                      console.log('saveEditPass error', error)
                    }
                }
            });
        },
        init () {
            ...
            if (user){
                this.userForm.name = user.staffName;
                this.userForm.cellphone = user.mobile;
                this.initPhone = user.mobile;
                this.userForm.email = user.email;
                this.userForm.company = user.deptName;
                this.userForm.department = user.positionName;
            }
            else {
